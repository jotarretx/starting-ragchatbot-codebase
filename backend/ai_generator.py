...
"""
    ...
    def generate_response(
        ...
        session = self._process_multi_round_tool_calling(
            base_messages, tools, tool_manager, rounds_limit
        )
    ...
    def _process_multi_round_tool_calling(
        self, base_messages: List[Dict], tools: List, tool_manager, max_rounds: int
    ) -> ToolCallSession:
        """
    ...
    def _make_api_call_with_tools(self, messages: List[Dict], tools: List) -> Dict:
        ...
        """
        api_params = {**self.base_params, "messages": messages, "tools": tools}
    ...
    def _execute_round_tools(
        self, round_obj: ToolCallRound, response: Dict, tool_manager
    ) -> ToolCallRound:
        """
        ...
        print(
            f"🔄 Round {round_obj.round_number}: AI decided to use {len(tool_calls)} tool(s)"
        )
    ...
    def _build_tool_result_messages(self, round_obj: ToolCallRound) -> List[Dict]:
        ...
        for i, tool_call in enumerate(round_obj.tools_called):
            tool_result = (
                round_obj.tool_results[i]
                if i < len(round_obj.tool_results)
                else "No result"
            )
    ...
    def _generate_final_response(self, session: ToolCallSession) -> str:
        ...
        if any_tools_used:
            ...
            return (
                first_round.ai_response["message"]["content"]
                if first_round
    ...
    def _log_session_summary(self, session: ToolCallSession):
        ...
        if total_tools == 0:
            ...
            for round_obj in session.rounds:
                if round_obj.has_tool_calls:
                    tools_summary = ", ".join(
                        [tool["name"] for tool in round_obj.tools_called]
                    )
                    print(f"   Round {round_obj.round_number}: {tools_summary}")
                else:
                    print(
                        f"   Round {round_obj.round_number}: Final response (no tools)"
                    )
